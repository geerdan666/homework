...
const value = prompt("Введите ваше число");
...
// тут никакие проверки не буду добавлять)
const username = prompt("Введите имя");
const password = Number(prompt("Введите пароль"));

const roles = ["admin", "user"];

if (roles.includes(username) && password === 123456) {
...
let parcelValue = prompt("Введите вес посылки (в килограммах)");
const deliveryType = prompt("Введите тип доставки");
const validTypes = ["Стандарт", "Экспресс", "Премиум"];

if (parcelValue === null || parcelValue.trim() === "") {
  alert("Нужно что-то ввести");
} else {
  let parcelWeight = Number(parcelValue);
  console.log(parcelWeight);
  if (isNaN(parcelWeight) || parcelWeight <= 0) {
    alert("Некорректный вес посылки");
  } else if (!validTypes.includes(deliveryType)) {
    alert("Неверный тип доставки");
  } else {
    // я сделаю тернарным тут
    const price =
      parcelWeight > 5 ? 15 : parcelWeight >= 1 && parcelWeight <= 5 ? 10 : 5;
    const coef =
      deliveryType === validTypes[0]
        ? 1
        : deliveryType === validTypes[1]
          ? 1.5
          : 2;

    alert(`Итоговая стоимость доставки: ${price * coef}$.`);
  }
}
